...
- `uv run kno serve` starts on :8000.
...
- `kno ingest hugo-repo dvhthomas/bitsby-me` → 100+ chunks indexed; visible in `/ui/kb`.
- `kno ingest drive-folder <folder-id>` with a folder containing 1 Doc, 1 Sheet, 1 PDF, 1 `.md` → all 4 files ingested with correct mime handling.
...
9. **Synthetic test tool**: `mcp:test_approval` with declared categories `read`, `external_write`, `external_messaging`, `irreversible` — used by integration tests. (No CLI approval surface — browser is canonical per v0.8.)
...
**OQ to resolve before P3 start:** OQ-11 (initial policy.yaml content). (OQ-12 dropped per v0.8.)
...
4. **Eval runner**: `kno eval <workflow>` runs every case against current version; LLM-as-judge using Haiku for rubric checks + a final natural-language judge prompt. Outputs pass/fail/cost table; persists to `eval_runs` + `eval_case_results`.
...
- `kno eval flow-coach` runs 5–10 cases against current `flow-coach@v1`, prints pass/fail with costs.
...
| **OQ-11** | Initial policy.yaml content | **P3** | Pre-write before P3 start: `kno policy lint` validates |
| ~~**OQ-12**~~ | ~~CLI approval UX~~ | ~~P3~~ | **Resolved (v0.8): dropped — browser-only approval.** |
