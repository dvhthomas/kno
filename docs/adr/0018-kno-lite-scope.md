...
11. **First-run experience: `kno setup` interactive wizard.** From a fresh clone, `uv run kno setup` walks through every `.env` value step-by-step — opens browser tabs to provider consoles at the right moments, prompts for paste-backs, verifies each value works before continuing (Anthropic API key, Ollama probes, GitHub OAuth callback), generates the Fernet KEK + session secret automatically, writes `.env` directly. Resumable: stops and `.env.partial` carries the in-progress state. ~15 minutes interactive vs. ~30+ minutes through the docs alone. **Setup that doesn't make you suffer is part of "you depend on it."** Costs ~1 dev-day; paid back the second time anyone (including future-you on a new machine) provisions Kno.
